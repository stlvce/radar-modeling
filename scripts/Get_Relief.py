...
# Инициализируем Sf как словарь для хранения параметров
Sf = {}
...
if Sf['Relief'] or 'Relief' not in locals():
    if Sf['Relief'] == 'peaks':
        Sf['x'] = Sf['Dspot']
        Sf['z'] = Sf['Dspot']
        # Создаем Relief как пиковую поверхность
        Relief = Sf['maxY'] * 0.5 * np.fromfunction(
            lambda z, x: np.sin(z) * np.cos(x), 
            (Sf['z'], Sf['x']), 
            dtype=float) + (0.5 - Sf['Elev'] / 100)
        ...
        Relief = (Sf['maxY'] * 
                  (0.5 * np.cos((np.real(XZ) - (Sf['n'] + 1) / 2) * Sf['rad_mul']) *
        ...
        Relief = np.load(relief_filepath)  # Здесь предполагается, что релief сохранен в .npy файле
    ...
    if Sf['maxY'] > 0:  # Сместим высоты и изменим масштаб
        ...
        if Sf['maxY'] == 0:
            Sf['maxY'] = difY
        Relief = (Relief - minY - difY * (Sf['Elev'] / 100)) * (Sf['maxY'] / difY) * 100 / (100 - Sf['Elev'])
