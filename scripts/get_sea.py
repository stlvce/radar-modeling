...
Sea['WindVec'] = rot_mat.dot(np.array([0, round(Sf['WindV']), 0]))
Sea['U10'] = np.linalg.norm(Sea['WindVec'])  # скорость ветра на высоте 10 м
Wind['V'] = np.array([0, Sf['WindV'], 0])
...
if Sf['WindV'] < 4:
    ...
    gamm = 3.3
    beta = 1.25
    fp = 3.5 * (consts['g'] ** 2 * Sea['fl'] * Sea['U10'] ** (-3)) ** (-0.33)
    
    # Разбиение частотного диапазона
    fa = int(Sea['nr'] * (fp - Sea['fs']) / (Sea['fe'] - Sea['fs']))
    fb = Sea['nr'] - fa
    Sea['F'] = np.concatenate([
        np.linspace(Sea['fs'], fp, fa),
        np.linspace(fp, Sea['fe'], fb)
    ])
    
    # Расчет спектральной плотности
    Sigma = np.where(Sea['F'] <= fp, 0.07, 0.09)
    r = np.exp(-0.5 * ((Sea['F'] - fp) / (fp * Sigma)) ** 2)
    alpha = 0.076 * (Sea['U10'] ** 2 / Sea['fl'] / consts['g']) ** 0.22
    S = alpha * consts['g'] ** 2 * (2 * np.pi) ** -4 * Sea['F'] ** -5 * \
        np.exp(-beta * (fp / Sea['F']) ** 4) * gamm ** r
    
    # Поправка на глубину
    F_ = Sea['F'] * np.sqrt(Sea['depth'] / consts['g'])
    ...
    # Расчет высоты волн
    kappa = 1
    Sea['H'] = np.sqrt(S * consts['g'] ** 2 / (8 * kappa * Sea['F'] ** 3))
...
Sea['G'] = consts['g'] + Sea['st'] / Sea['rho'] * Sea['K'] ** 2
Sea['W'] = np.sqrt(Sea['G'] * Sea['K'] * np.tanh(Sea['K'] * Sea['depth']))  # Угловые частоты
Sea['V_wave'] = Sea['H'] ** 2 * (2 * np.pi / Sea['L']) ** 2 * Sea['W']  # Скорости волн
...
for n in range(Sea['nr']):
    D = (X - Sea['V_wave'][n] * t) * np.cos(Sea['Dir'][n]) + \
        (Y - Sea['V_wave'][n] * t) * np.sin(Sea['Dir'][n])
