...
# Предполагаем, что Get_Relief.py создаёт Relief и сохраняет данные в переменной Relief
from Get_Relief import Relief  # Импортируем данные о рельефе из другого скрипта
...
try:
    tmp = plt.get_current_fig_manager().window.winfo_width(), plt.get_current_fig_manager().window.winfo_height()
...
# Создаём сетку
X, Z = np.meshgrid((np.arange(1, Sf['x_size'] + 1) - Sf['med_shift']),
...
plt.title(f'Вид рельефа "{Sf["Relief"]}"; высоты от {int(mm[0])} до {int(mm[1])} м', fontsize=9, fontweight='normal')
