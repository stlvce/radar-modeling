...
Nimp = int(evs['Rs']['Nimp'])

if Ym == 0:
    tauimp = evs['Rs']['tauimp']
    Timp = evs['Rs']['Timp']
    dtau = evs['Rs']['dtau']
    Rs.Tm = Rs.Timp
    Tm = Timp
...
if 'Tr' in locals() and len(Tr.Pos) > 0:
    ...
    # Начальные значения
    Tr.Pos[0, :] = [evs['Tr']['Xa'], evs['Tr']['Ya'], evs['Tr']['Za']]
    St.Pos = np.zeros((Tr.N + 1, 3, St.N))
    St.Pos[0, :, n] = [evs['St']['Xs'], evs['St']['Ys'], evs['St']['Zs']]
...
consts = {
    ...
    'TrVskip': np.array_equal(consts['TrVall'], '000'),
    'StVall': np.array([St.Vx, St.Vy, St.Vz]),
    'StVskip': np.array_equal(consts['StVall'], '000')
}
...
for m in range(Tr.N + 1):
    ...
    H = Tr.Pos[m, 1]  # высота
    Tr.V[m, :] = [evs['Tr']['Vx'], evs['Tr']['Vy'], evs['Tr']['Vz']]
    Tr.Ang[m, :] = np.radians([evs['Tr']['tang'], evs['Tr']['kren'], evs['Tr']['psi']])

    if St.N > 0:  # расчет точек траектории всех целей
        St.V[m, 0, n] = evs['St']['Vx']
