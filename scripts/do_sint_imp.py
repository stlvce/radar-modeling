...
def init_radar_impulse_processor_globals(globals: dict[str, Any] | None = None,):
    """
...
def process_radar_impulse(globals: dict):
    ...
    for ChCnt in range(ChannelN):
        ...
        for RCounter in range(RCmax):
            ...
            indices = np.minimum(1 + NLr + np.arange(Nimp) * NLb, ScosNN.shape[1] - 1).astype(int)
...
def plot_radar_impulse_results(globals: dict):
    ...
    for i, ChCnt in enumerate(ch_indices):
        ...
        if Rs['Log']:
            ...
            plt.imshow(RLI1f, extent=[
                Dop[0] * 1e-3, Dop[-1] * 1e-3,
                Dal[0], Dal[-1]
            ], aspect='auto')
